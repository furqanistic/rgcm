...
export default function ViewDeletedBookings() {
  ...
  const { data, status } = useQuery('stats-all-deleted', async () => {
    const res = await axiosInstance.get(`/booking/all-deleted`)
    return res.data
  })
  ...
  const filteredRows = data
    ...
    .map((item) => {
      ...
      return {
        ...
        contact: item.contact,
        location: item.location,
        date: item.date, // Adjusted to use the booking date
        timings: item.timings,
        totalAmount: item.totalAmount,
        functionCat: item.functionCat,
        formType: item.formType,
    ...
    <ThemeProvider theme={darkTheme}>
      <div style={{ height: '100%', width: '100%' }}>
        <FiltersContainer>
          ...
          <Link style={{ textDecoration: 'none' }} to='/find/all'>
            <GenBtn>
